...
// get posts by filter, if filter is empty return all posts
export const getPostsByFilter = async (filter) => {
  try {
    await connectToDatabase();

    // filtrar publicaciones por su campo referencia
    const posts = await Post.find({ reference: filter.filter })
      .populate("user")
      .populate("comments")
      .sort({ createdAt: -1 });
...
export const getPostsByType = async (type) => {
  try {
    ...
    // filtrar publicaciones por su campo referencia
    const posts = await Post.find({ type: type.type })
      .populate("user")
      .populate("comments")
      .sort({ createdAt: -1 });
